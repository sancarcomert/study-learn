...
- ✅ **İstatistik alanı — çaba/süreklilik** (`ae0a8da`, cihazda test): rakip + TR lise 9-12 kitle araştırması sonrası. **BU HAFTA** (tamamlanan görev · aktif gün X/7 · işaretli konu 3 kart) · **AKTİF GÜNLER** 12 haftalık ısı haritası (`widgets/activity_heatmap.dart`, GitHub tarzı, `completedAt ?? dueDate`) · **DERS DAĞILIMI** (bu hafta/ay toggle, ders başına tamamlanan görev bar). "SON 7 GÜN" bar kaldırıldı (ısı haritası kapsıyor), `fl_chart` importu stats_screen'den çıktı. Hepsi mevcut provider'dan türetiliyor, yeni model yok. **Sıradaki (B):** odak seansı geçmişi — `FocusSession` Hive modeli (tarih+dakika) → odak trendi grafiği. `focusMinutes` şu an tarihsiz kümülatif.
