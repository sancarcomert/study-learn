...
| P0-5 | **Odak kronometresi arka planda + kalıcı bildirim** ✅ (2026-09-26, `NotificationService.showOngoingFocus` — native chronometer, `focus_screen.dart` lifecycle kancaları). | Rakiplerin #1 övgüsü ("uygulama kapalıyken de sayıyor"). | Orta (foreground service YERİNE Android native chronometer bildirimi — tam foreground service değil ama aynı kullanıcı deneyimini daha az riskle veriyor) |
| P0-6 | **Odak seansı: duraklat** ✅ (zaten vardı) **+ geçmişi düzenle** ✅ **/sil** ✅ (2026-09-26, `focus_history_screen.dart._EditSessionSheet`). | Rakiplerin #1 eksik-özellik şikayeti. | Orta |
